...
var CHU = {
  vi: {
    ...
    dxBoKiemMo: 'Đề xuất này được duyệt thẳng, không qua bước kế toán kiểm số. Luồng cho phép, nhưng lịch sử ghi lại để người soát về sau phân biệt được với đề xuất đã qua kiểm.',
    dxKyMoiMo: 'Bên này chưa có bản ghi hợp đồng nào trong hệ — đây là ký mới, không phải gia hạn. Deal ký mới thuộc quy trình CRM; nếu nó đã qua CRM thì nhận từ CRM để giữ lại chuỗi duyệt đã có.',
  ...
  en: {
    ...
    dxBoKiemMo: 'This proposal was approved directly, without the accounting check. The flow allows it, but the history records it so a later reviewer can tell it apart from one that was checked.',
    dxKyMoiMo: 'This party has no contract record in the system — this is a new signing, not a renewal. New signings belong to the CRM process; if this one already went through the CRM, receive it from there so its approval chain survives.',
...
function theDeXuat(pr, opts) {
  ...
  /* KÝ MỚI hay GIA HẠN: hai quyết định khác hẳn nhau, nên nói ngay ở hàng
     huy hiệu chứ đừng để người duyệt tự suy từ ô "phí hiện tại".
     `viec` do lõi suy ra từ việc bên đã có bản ghi hợp đồng hay chưa —
     không phải từ ô ghi chú tự do, thứ mà cả ba đề xuất mẫu đều ghi
     "Gia hạn trước hạn" kể cả khi không có gì để gia hạn. */
  /* gia hạn là ca thường, để huy hiệu trung tính; ký mới mới cần chú ý */
  var tagViec = c.viec === 'moi' ? HM.tag(t('dxKyMoi'), 'warn')
              : c.viec === 'gia-han' ? HM.tag(t('dxGiaHan'), '') : '';
  var tagBoKiem = pr.boQuaKiem ? HM.tag(t('dxBoKiem'), 'warn') : '';
  var dau = '<div class="bar" style="margin-bottom:10px">' + tagDx(pr.status) + tagViec + tagBoKiem + tagKn(c.recommendation) + (c.grade ? tagHang(c.grade) : '') +
  ...
  if (pr.type === 'advance') {
    ...
    html += '<div class="sig">' +
      ...
      /* Không có hợp đồng thật thì ngày hết hạn là SUY RA từ mã bên, không
         phải một dữ kiện. Nói ra, và đừng tô đỏ "sắp hết hạn" cho một ngày
         mình tự nghĩ ra: 138 bên đang nằm trong hàng đợi sắp-hết-hạn mà chỉ
         2 bên có bản ghi hợp đồng. */
      neu(c.contractEnd, function () { return oSo(t('dxEnd'), HT.fmt.date(c.contractEnd),
        c.hanSuyRa ? t('dxHanSuyRa') : (c.renewalDue ? t('dxDue') : null), c.renewalDue && !c.hanSuyRa); }) +
      '</div>' +
      (c.viec === 'moi' ? '<p class="hint" style="margin-top:10px">' + esc(t('dxKyMoiMo')) + '</p>' : '') +
  ...
  html += '<h4 class="sec" style="margin-top:14px">' + esc(t('dxHistory')) + '</h4><ul class="tl">' + pr.history.slice().reverse().map(function (h) {
    return '<li>' + tagDx(h.status) + (h.boQuaKiem ? ' ' + HM.tag(t('dxBoKiem'), 'warn') : '') + ' <span class="mono" style="font-size:12px">' + esc(h.at) + '</span>' + (h.by ? ' · ' + esc(h.by) : '') + (h.note ? '<div class="muted" style="font-size:12.5px">' + esc(song(h, 'note')) + '</div>' : '') + (h.boQuaKiem ? '<div class="muted" style="font-size:12.5px">' + esc(t('dxBoKiemMo')) + '</div>' : '') + '</li>';
  }).join('') + '</ul>';
  return html;
}
