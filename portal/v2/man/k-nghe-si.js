...
HT.dangKy({
  ...
  chu: {
    vi: {
      ...
      mo: 'Từng nghệ sĩ trong roster mang về bao nhiêu trong kỳ, phần Haustek thanh toán cho nghệ sĩ và phần label được hưởng.',
      soNs: 'Nghệ sĩ trong roster', coDt: 'Có doanh thu kỳ này', gop: 'Doanh thu gộp roster',
      traNs: 'Thanh toán cho nghệ sĩ', labelHuong: 'Phần label được hưởng',
      tim: 'Tìm tên hoặc mã nghệ sĩ…',
      cNs: 'Nghệ sĩ', cBai: 'Bài có doanh thu', cLuot: 'Lượt nghe', cGop: 'Doanh thu gộp',
      cNsHuong: 'Phần nghệ sĩ', cLabel: 'Phần label', cTyLe: 'Tỷ lệ',
      topNs: 'Nghệ sĩ mang về nhiều nhất kỳ này', topNsMo: 'Theo doanh thu gộp. Bấm một dòng trong bảng để xem chi tiết.',
      khongCo: 'Chưa có nghệ sĩ nào khớp', khongCoMo: 'Thử tìm bằng mã khách hàng của nghệ sĩ.',
      chuaMo: 'Kỳ này chưa chốt sổ', chuaMoMo: 'Số liệu theo nghệ sĩ chỉ hiện sau khi kỳ được xét duyệt.',
      ghiChu: 'Phần nghệ sĩ là số Haustek thanh toán thẳng cho nghệ sĩ theo tỷ lệ label đã đặt; phần label là phần còn lại sau phí dịch vụ Haustek. Cộng phần nghệ sĩ, phần label và điểm producer thì bằng doanh thu gộp trừ phí dịch vụ.',
      xuat: 'Xuất CSV', hienThi: 'Hiển thị',
      chiTiet: 'Bài của nghệ sĩ này trong kỳ', khongBai: 'Kỳ này nghệ sĩ chưa có bài nào phát sinh doanh thu.',
    ...
    en: {
      ...
      soNs: 'Artists on the roster', coDt: 'Earning this period', gop: 'Roster gross revenue',
      traNs: 'Paid to artists', labelHuong: 'Label’s share',
      tim: 'Search artist name or code…',
      cNs: 'Artist', cBai: 'Earning tracks', cLuot: 'Streams', cGop: 'Gross',
      cNsHuong: 'Artist share', cLabel: 'Label share', cTyLe: 'Rate',
      topNs: 'Top artists this period', topNsMo: 'By gross revenue. Open a row in the table for details.',
      ...
      ghiChu: 'The artist share is what Haustek pays the artist directly at the rate the label set; the label share is the remainder after the Haustek service fee. Artist share, label share and producer points add back to gross less the fee.',
  ...
  ve: function (root, c) {
    ...
    html += HM.so([
      { l: t('gop'), v: HT.fmt.usd0(r.total.gross), lon: true },
      { l: t('traNs'), v: HT.fmt.usd0(r.total.artist), s: hd ? HT.fmt.pct(hd.artistShare) + (c.lang === 'vi' ? ' sau phí dịch vụ' : ' after the fee') : '' },
      { l: t('labelHuong'), v: HT.fmt.usd0(r.total.labelCut), s: hd ? HT.fmt.pct(hd.counterpartShare) + (c.lang === 'vi' ? ' sau phí dịch vụ' : ' after the fee') : '', mau: HB.mau('ok') },
      { l: t('soNs'), v: HT.fmt.n(r.count), s: HT.fmt.n(r.earning) + ' ' + (c.lang === 'vi' ? 'có doanh thu' : 'earning') }
    ]);

    var top = r.rows.filter(function (x) { return x.gross > 0; }).slice(0, 8);
    if (top.length) {
      html += HM.the({
        h2: HM.esc(t('topNs')), p: HM.esc(t('topNsMo')),
        than: HB.o({ loai: 'thanh', tenTong: t('cGop'), hang: top.map(function (x, i) {
          return { ten: x.name, gt: x.gross, phu: x.clientId, mau: P[i % 8] };
        }) })
      });
    }
    ...
    html += HM.the({
      ...
      than: rows.length ? '<div class="tw"><table class="t"><thead><tr>' +
        ...
        cot('gross', t('cGop'), true) + cot('artist', t('cNsHuong'), true) + cot('labelCut', t('cLabel'), true) +
        ...
        '<td class="num">' + HM.esc(HT.fmt.usd(rows.reduce(function (s, x) { return s + x.gross; }, 0))) + '</td>' +
    ...
    HM.bam(root, '[data-xuat]', function () {
      HM.csv('nghe-si-' + me.clientId + '-' + c.kyKey + '.csv',
        ...
        rows.map(function (x) { return [x.clientId, x.name, x.tracks, x.streams, x.gross.toFixed(2), x.artist.toFixed(2), x.labelCut.toFixed(2), x.producer.toFixed(2)]; }));
    });
...
function moNgheSi(c, x) {
  ...
  var api = c.api, me = c.phien.me, t = c.t, P = HB.dayMau();
  var bg;
  try { bg = api.tracks(me.role, me.partyId, c.kyKey, 'rec', { q: x.name, sort: 'gross', dir: -1 }); }
  catch (e) { bg = { rows: [], total: 0 }; }
  var rows = bg.rows.filter(function (r) { return r.artist === x.name; }).slice(0, 12);
  c.nganTruot(
    HM.so([
      { l: t('cGop'), v: HT.fmt.usd(x.gross), lon: true },
    ...
    (rows.length ? '<div class="tw"><table class="t" style="min-width:0"><thead><tr><th>' + HM.esc(c.lang === 'vi' ? 'Bài hát' : 'Track') + '</th>' +
      ...
      rows.map(function (r) {
        return '<tr><td><div class="t-ttl">' + HM.esc(HM.dai(r.title, 34)) + '</div><div class="t-sub">' + HM.esc(r.isrc) + '</div></td>' +
          '<td class="num">' + HM.esc(HT.fmt.n(r.streams)) + '</td><td class="num">' + HM.esc(HT.fmt.usd0(r.gross)) + '</td>' +
